...
/**
 * Simple non-recursive transform of a primitive object
...
/**
 * Simple non-recursive transform of a delimited string of key value pairs
 * to an object representation
 *
 * @param source the string to be transformed
 * @param seperator the token used as key/value pairs seperator
 * @param equals the token used to separate the key and value
 */
export const stringToObject = (source: string, seperator: string = "&", equals: string = "=") => {
  const o = {}
  if (source) {
    source.split(seperator).forEach((keyValue: string) => {
      const parts = keyValue.split(equals)
      if (parts.length) {
        Object.assign(o, {
          [parts[0]]: parts[1] || "",
        })
